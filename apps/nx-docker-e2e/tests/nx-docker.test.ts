...
describe('nx-docker e2e', () => {
  ...
  describe('--directory', () => {
    it('should create src in the specified directory', async (done) => {
      const plugin = uniq('nx-docker');
      ensureNxProject('@gperdomor/nx-docker', 'dist/libs/nx-docker');
      await runNxCommandAsync(`generate @gperdomor/nx-docker:nxDocker ${plugin} --directory subdir`);
  ...
  describe('--tags', () => {
    it('should add tags to nx.json', async (done) => {
      const plugin = uniq('nx-docker');
      ensureNxProject('@gperdomor/nx-docker', 'dist/libs/nx-docker');
      await runNxCommandAsync(`generate @gperdomor/nx-docker:nxDocker ${plugin} --tags e2etag,e2ePackage`);
