...
<Tabs items={['Docker', 'Podman','Kaniko']}>
  <Tab value="Docker">
  ...
  <Tab value="Kaniko">
    ...
    ```yml title=".gitlab-ci.yml"
    build-with-kaniko:
      image: gperdomor/nx-kaniko:24.0.0-alpine
      variables:
        # Nx Container
        INPUT_PUSH: 'true' # To push your image to the registry
    ...
    <Callout title="Tip!" type="info">
      Kaniko don't support parallel builds, so we need to build one application at time with `--parallel=1`
    </Callout>
